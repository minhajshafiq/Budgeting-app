...
class TransactionService {
  ...
  /// Créer des transactions récurrentes de test (pour le développement)
  Future<void> createTestRecurringTransactions() async {
    if (_cachedTransactions.isNotEmpty) {
      debugPrint('Des transactions existent déjà, pas de création de données de test');
      return;
    }

    final now = DateTime.now();
    
    // Transaction mensuelle (loyer)
    ...
    // Transaction hebdomadaire (courses)
    final groceriesTransaction = Transaction(
      title: 'Courses',
      amount: 120.0,
    ...
    // Transaction quotidienne (café)
    final coffeeTransaction = Transaction(
      title: 'Café',
      amount: 3.50,
      date: DateTime(now.year, now.month, now.day - 1),
      categoryId: 'expense_food',
      type: TransactionType.expense,
      description: 'Café quotidien',
      recurrence: RecurrenceType.daily,
    );

    // Transaction annuelle (assurance)
    final insuranceTransaction = Transaction(
    ...
    // Transaction trimestrielle (électricité)
    final electricityTransaction = Transaction(
      title: 'Électricité',
      amount: 180.0,
      date: DateTime(now.year, now.month, 20),
      categoryId: 'expense_bills',
      type: TransactionType.expense,
      description: 'Facture trimestrielle',
      recurrence: RecurrenceType.quarterly,
    );

    // Ajouter toutes les transactions de test
    await addTransaction(rentTransaction);
    await addTransaction(groceriesTransaction);
    await addTransaction(coffeeTransaction);
    await addTransaction(insuranceTransaction);
    await addTransaction(electricityTransaction);

    debugPrint('5 transactions récurrentes de test créées');
  }
  ...
  Future<void> _loadTransactionsFromStorage() async {
    try {
      ...
      if (transactionsJson != null) {
        ...
        // Pas de données - créer des transactions récurrentes de test
        _cachedTransactions = [];
        await createTestRecurringTransactions();
        debugPrint('Nouveau compte initialisé avec des transactions récurrentes de test');
      }
    } catch (e) {
